...
export interface B2AccessTestResult {
  success: boolean;
  bucketName: string;
  endpoint: string;
  region: string;
...
/**
 * Returns summary of runtime Backblaze B2 configuration without exposing sensitive secrets.
 */
...
/**
 * Backblaze B2 S3 Client Factory.
 */
export function createB2S3Client(): {
  ...
  const client = new S3Client({
    ...
    // Backblaze B2 S3 API supports path-style requests reliably across all regions
    forcePathStyle: true,
  });

  return { client, bucketName, endpoint, region };
}

/**
 * Live connectivity check for Backblaze B2.
 * Tests bucket reachability and presigning capability.
 */
export async function testB2Access(): Promise<B2AccessTestResult> {
  const config = getB2ConfigStatus();

  if (!config.hasKeyId || !config.hasApplicationKey || !config.hasBucketName) {
    const missing: string[] = [];
    if (!config.hasKeyId) missing.push('B2_KEY_ID');
    if (!config.hasApplicationKey) missing.push('B2_APPLICATION_KEY');
    if (!config.hasBucketName) missing.push('B2_BUCKET_NAME');
    ...
    return {
      ...
      canReadBucket: false,
      canPresign: false,
      error: `Missing required Backblaze environment variables: ${missing.join(', ')}`,
    };
  }

  try {
    const b2 = createB2S3Client();
    if (!b2) {
      return {
        success: false,
        ...
        canReadBucket: false,
        canPresign: false,
        error: 'Failed to construct Backblaze B2 S3 client with current credentials.',
      };
    }

    // 1. Test live bucket access via ListObjectsV2Command (MaxKeys: 1)
    console.log(`[Backblaze B2:Test] 🔍 Testing bucket access on "${b2.bucketName}"...`);
    const listRes = await b2.client.send(
    ...
    const canReadBucket = listRes.$metadata?.httpStatusCode === 200;

    // 2. Test presigned URL generation (private access verify)
    const testCommand = new GetObjectCommand({
      Bucket: b2.bucketName,
      Key: 'test-connectivity-check.mp4',
    });

    const samplePresignedUrl = await getAwsSignedUrl(b2.client, testCommand, {
      expiresIn: 300,
    });

    const canPresign = Boolean(samplePresignedUrl && samplePresignedUrl.startsWith('http'));

    console.log(
      `[Backblaze B2:Test] ✅ Backblaze B2 connected successfully! (Bucket: ${b2.bucketName}, Region: ${b2.region})`
    );

    return {
      success: canReadBucket && canPresign,
      bucketName: b2.bucketName,
      endpoint: b2.endpoint,
      region: b2.region,
      canReadBucket,
      canPresign,
      samplePresignedUrl: samplePresignedUrl ? samplePresignedUrl.substring(0, 80) + '...' : undefined,
      message: `Successfully connected to Backblaze B2 bucket "${b2.bucketName}" (${b2.region}). S3 credentials and presigning verified.`,
    };
  } catch (err: any) {
    const errorMsg = err?.message || String(err);
    console.error('[Backblaze B2:Test] ❌ Connection test failed:', errorMsg);
    return {
      success: false,
      bucketName: config.bucketName,
      endpoint: config.endpoint,
      ...
      canReadBucket: false,
      canPresign: false,
      error: `Backblaze B2 connection test failed: ${errorMsg}`,
    };
  }
}
 ...
 * - NO permanent public URLs (all bucket objects remain private)
...
export class B2StorageProvider implements IStorageProvider {
  ...
  constructor() {
    ...
    if (keyId && applicationKey && this.bucketName) {
      ...
      console.warn(
        '[Backblaze B2] ⚠️ Missing credentials: B2_KEY_ID, B2_APPLICATION_KEY, or B2_BUCKET_NAME not set.'
      );
    }
  }
  ...
  private getClient(): S3Client {
    if (!this.s3Client) {
      throw new Error(
        'Backblaze B2 is not configured. Please ensure B2_KEY_ID, B2_APPLICATION_KEY, and B2_BUCKET_NAME are set in environment variables.'
      );
    }
 ...
 */
export class BackblazeB2StorageProvider extends B2StorageProvider {}
export class R2StorageProvider extends B2StorageProvider {}
